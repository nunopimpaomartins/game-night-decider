...
from sqlalchemy import select, delete, update as sa_update, func
...
async def set_bgg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Linked BGG account: {bgg_username}. Fetching collection{mode_text}...")
    ...
    try:
        ...
        await update.message.reply_text("Failed to fetch collection from BGG. The service might be temporarily unavailable. Please try again later.")
...
async def start_night(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        if db_session and db_session.is_active:
            players_stmt = select(SessionPlayer).where(SessionPlayer.session_id == chat_id).options(selectinload(SessionPlayer.user))
...
async def join_lobby_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"📢 {query.from_user.first_name} has joined the game night!"
        )
...
async def leave_lobby_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"👋 {query.from_user.first_name} has left the game night."
        )
...
async def create_poll(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        total_games_query = (
            select(func.count(Collection.game_id.distinct()))
            .where(Collection.user_id.in_(player_ids))
        )
    ...
    if not valid_games:
        await update.message.reply_text(f"No games found matching {player_count} players (intersection of collections).")
        return
...
async def mark_played(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        if len(games) > 1:
            await update.message.reply_text(f"Found multiple games: {', '.join([g.name for g in games])}. Be more specific.")
            return
...
async def exclude_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        col_stmt = select(Collection).where(Collection.user_id == user_id, Collection.game_id == game.id)
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        stmt = (
            ...
            .where(
                ...
                Game.name.ilike(f"%{game_name_query}%")
            )
        )
        ...
        exact_match = next((pair for pair in collections_games if pair[1].name.lower() == game_name_query.lower()), None)
        ...
        if exact_match:
            ...
            if new_state:
                await update.message.reply_text(f"⭐ Game '{target_game.name}' is now HIGH PRIORITY!")
            else:
            ...
            for col, game in collections_games[:10]: # Limit to 10 buttons
                keyboard.append([
                    InlineKeyboardButton(game.name, callback_data=f"prio:{game.id}")
                ])
...
async def priority_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db.AsyncSessionLocal() as session:
        col_stmt = select(Collection, Game).join(Game).where(
            Collection.user_id == user_id, 
            Collection.game_id == game_id
        )
...
async def add_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        exact_match = next((g for g in results if normalize_name(g["name"]) == search_normalized), None)
...
async def test_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        try:
            ...
            await update.message.reply_text("Usage: /testmode [number_of_players]\nExample: /testmode 4")
            return
    ...
    try:
        async with db.AsyncSessionLocal() as session:
            ...
            test_games = [
                Game(id=-1001, name="Test Catan", min_players=3, max_players=4, playing_time=60, complexity=2.3),
                Game(id=-1002, name="Test Ticket to Ride", min_players=2, max_players=5, playing_time=45, complexity=1.8),
                Game(id=-1003, name="Test Wingspan", min_players=1, max_players=5, playing_time=60, complexity=2.4),
                Game(id=-1004, name="Test Gloomhaven", min_players=1, max_players=4, playing_time=120, complexity=3.9),
            ]
            ...
            if not calling_user:
                calling_user = User(telegram_id=calling_user_id, telegram_name=update.effective_user.first_name)
                session.add(calling_user)
            
            # Check if calling user is already in session (re-add safety)
            sp_stmt = select(SessionPlayer).where(
                SessionPlayer.session_id == chat_id, 
                SessionPlayer.user_id == calling_user_id
            )
...
async def guest_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(numeric_values) >= 1: min_players = int(float(numeric_values[0]))
    if len(numeric_values) >= 2: max_players = int(float(numeric_values[1]))
    if len(numeric_values) >= 3: complexity = float(numeric_values[2])
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        if not matched_guest:
            ...
            await update.message.reply_text(f"Could not find a matching guest in: {full_text}.\nActive guests: {', '.join(names)}")
            return
            
        guest_display_name = matched_guest.telegram_name
            
        if not game_name_str:
            await update.message.reply_text(f"Found guest '{guest_display_name}' but no game name provided.")
            return
        ...
        col_stmt = select(Collection).where(
            Collection.user_id == matched_guest.telegram_id,
            Collection.game_id == game.id
        )
        if not (await session.execute(col_stmt)).scalar_one_or_none():
    ...
    await update.message.reply_text(f"Added '{final_game_name}' to {guest_display_name}'s collection!")
...
async def start_poll_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with db.AsyncSessionLocal() as session:
        ...
        if not players:
            await context.bot.send_message(
                chat_id=chat_id,
                text="No players in lobby! Click Join first."
            )
            return
        ...
        if len(players) < 2:
            await context.bot.send_message(
                chat_id=chat_id,
                text="Need at least 2 players to start a poll!"
            )
            return
    ...
    if not valid_games:
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"No games found matching {player_count} players."
        )
        return
